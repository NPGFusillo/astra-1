...
def line_func_rv(params, _sn, _l, pca_model, wref):
    import numpy as np
    from scipy import interpolate
    ...
    recovered = reconstruct_spectrum(_T, _g, pca_model)
    ...
    for i in range(len(_l)):
        ...
        #line_resid = (f_model-f_obs) / e_obs#e_obs
        r=(f_model-f_obs) / e_obs
        #residuals.append(line_resid)
        residuals.append(np.sign(r)*np.sqrt(np.abs(r)))
    if len(residuals) == 0:
...
def reconstruct_spectrum(teff, logg,pca_model):
    interp_coeffs = pca_model["interp_coeffs"]
    pca = pca_model["pca"]
    K = pca.n_components_
    c = np.array([
        interp_coeffs[k](teff, logg)[0, 0]
        for k in range(K)
    ])

    log_flux = pca.mean_ + np.dot(c, pca.components_)
